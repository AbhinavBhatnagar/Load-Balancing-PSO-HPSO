function []=dHpso(machine,request,oldreq,pointer,fig,igraphs);
xmax=10;
xmin=1;
vmax=1;
vmin=-1;
...
if(strcmp(check,'yes'))
   ...
   % disp(oldreq);
    dHpso(machine,request+extra,oldreq,pointer,fig,igraphs);
    %disp(interrupt);
...
fig(pointer)=gbest;
if(pointer>=2)
 if(fig(pointer-1)>fig(pointer))
   fig(pointer)=fig(pointer-1);
end
end
igraphs(pointer)=minutz*10;
if(pointer>=2)
